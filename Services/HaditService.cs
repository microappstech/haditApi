...
namespace haditApi
{
    public class HaditService
    {
        ...
        public HaditService(dbContext dbContext)
        {
        ...
        public ApiResponse<List<Category>> GetCategories()
        {
            try
            {
                var cates = _dbContext.Categories.ToList();
        ...
        public ApiResponse<List<Hadit>> SearchByContent(string name)
        {
            try
            {
                ...
                var _hadits = _dbContext.Hadites.ToList()
                    .Where(i => !string.IsNullOrEmpty(RemoveDiacritics(i.Content)))
                    .ToList();


                var hadits = _dbContext.Hadites.ToList()
                    .Where(i => RemoveDiacritics(i.Content).Contains(normalizedInput))
                    ?.ToList();
        ...
        public ApiResponse PostHadites(List<Hadit> hadits)
        {
            try
            {
                _dbContext.AddRange(hadits);
